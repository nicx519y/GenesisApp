...
class _OriginFeedState extends State<_OriginFeed> {
  ...
  Future<List<_OriginCardVm>> _fetchItems() async {
    ...
    final page = await api.getOrigins(category: widget.category, limit: 20, offset: 0);
    return page.data
        .map((o) => _OriginCardVm(item: _toOriginItem(o), originId: o.id, oid: o.oid))
        .toList(growable: false);
  }

  OriginItem _toOriginItem(OriginSummary origin) {
    final hash = origin.oid.codeUnits.fold<int>(0, (a, b) => (a * 31 + b) & 0x7fffffff);
  ...
  String _badgeText(String name) {
    ...
    final words = cleaned.split(RegExp(r'\s+')).where((e) => e.trim().isNotEmpty).toList();
    if (words.isEmpty) return 'ENTER\nWORLD';
    return words.take(4).map((e) => e.toUpperCase()).join('\n');
  }

  List<Color> _gradientFor(String seed) {
    final hash = seed.codeUnits.fold<int>(0, (a, b) => (a * 131 + b) & 0x7fffffff);
