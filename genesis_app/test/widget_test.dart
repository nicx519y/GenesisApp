...
void main() {
  ...
  testWidgets('origin detail version 2 uses root Tilemap endpoint', (
    ...
    final expandedComposerVisibility = find.byKey(
      const ValueKey<String>('origin-expanded-opening-composer-visibility'),
    );
    expect(expandedComposerVisibility, findsOneWidget);
    expect(
      tester.widget<IgnorePointer>(expandedComposerVisibility).ignoring,
      isTrue,
    );
    expect(
  ...
  testWidgets('origin opening location scrolls with the Worldo brief', (
    ...
    final openingScrollController = tester
    ...
    final locationTopBeforeScroll = tester.getTopLeft(openingLocation).dy;
    final scrollDelta = openingScrollController.position.maxScrollExtent
        .clamp(1.0, 24.0)
        .toDouble();
    openingScrollController.jumpTo(
      openingScrollController.offset + scrollDelta,
    );
    await tester.pump();
    expect(
      tester.getTopLeft(openingLocation).dy,
      closeTo(locationTopBeforeScroll - scrollDelta, 1),
    );
  });
  ...
  testWidgets('Origin opening puts the recommended role first and marks it', (
    ...
    expect(
      roleCards.controller!.viewportFraction,
      closeTo(252 / tester.getSize(roleCardsFinder).width, 0.001),
    );
    ...
    expect(
      find.byKey(const ValueKey<String>('origin-setup-role-edit-c_regular')),
      findsNothing,
    );
    ...
    expect(
      tester
          .getCenter(
            find.byKey(
              const ValueKey<String>(
                'origin-setup-role-card-frame-c_recommended',
              ),
            ),
          )
          .dx,
    ...
    unawaited(
      roleCards.controller!.animateToPage(
        1,
    ...
    await tester.pumpAndSettle();
    final regularFrame = tester.widget<Container>(
      find.byKey(
        const ValueKey<String>('origin-setup-role-card-frame-c_regular'),
      ),
    );
    final regularOutline = regularFrame.foregroundDecoration! as BoxDecoration;
    expect(
      tester
          .getCenter(
            find.byKey(
  ...
  testWidgets('Origin detail role setup launches without a bottom bar', (
    ...
    expect(setupAvatarSource.imageUrl, contains('image/resize,w_360'));
    ...
    final detailsBackground = tester.widget<ColoredBox>(
      find.byKey(
    ...
    expect(
      find.descendant(of: details, matching: find.text('Personality')),
      findsOneWidget,
    );
    expect(
    ...
    expect(infoAvatar.imageUrl, contains('image/resize,w_90'));
    sheetPagesRect = tester.getRect(sheetPages);
    await tester.dragFrom(
      Offset(sheetPagesRect.left + 24, sheetPagesRect.top + 16),
      Offset(sheetPagesRect.width * 0.8, 0),
  ...
  testWidgets('Origin shows cached signed-in profile as a direct launch role', (
    ...
    final selectedProfileFrame = tester.widget<Container>(
      find.byKey(
    ...
    expect(profileAvatarSource.imageUrl, contains('image/resize,w_720'));
    expect(profileAvatarSource.cacheWidth, 480);
    expect(profileAvatarProvider.outputSize, 480);
    ...
    expect(
      find.descendant(of: profileDetails, matching: find.text('Personality')),
      findsOneWidget,
    );
    expect(
    ...
    await _dragOriginPanelUntilVisible(tester, editSurfaceFinder);
    final sheetSurface = find.byKey(
      const ValueKey<String>('origin-detail-sheet-surface'),
    );
    final sheetTopBeforeEdit = tester.getTopLeft(sheetSurface).dy;
    tester
        .widget<InkWell>(
          find.byKey(
    ...
    final inlineEditor = find.byKey(
      const ValueKey<String>('origin-setup-role-inline-editor-current-user'),
    );
    expect(inlineEditor, findsOneWidget);
    expect(tester.getSize(inlineEditor), const Size(240, 288));
    expect(tester.getTopLeft(sheetSurface).dy, sheetTopBeforeEdit);
    final inlineAvatar = find.byKey(
      const ValueKey<String>('origin-setup-role-inline-avatar-current-user'),
    );
    expect(tester.getSize(inlineAvatar), const Size.square(104));
    expect(
      tester.getCenter(inlineAvatar).dx,
      closeTo(tester.getCenter(inlineEditor).dx, 0.01),
    );
    final inlineAvatarImage = tester.widget<GenesisStaticNetworkImage>(
      find.descendant(
        of: inlineAvatar,
        matching: find.byType(GenesisStaticNetworkImage),
      ),
    );
    expect(inlineAvatarImage.imageUrl, contains('image/resize,w_360'));
    expect(
      inlineAvatarImage.maxDevicePixelRatio,
      originWorldOpeningRoleAvatarMaxDevicePixelRatio,
    );
    final removeAvatar = find.descendant(
      of: inlineAvatar,
      matching: find.byKey(const ValueKey<String>('create-upload-remove')),
    );
    expect(removeAvatar, findsOneWidget);
    tester.widget<IconButton>(removeAvatar).onPressed!();
    await tester.pump();
    final emptyAvatarSurface = tester.widget<Container>(
      find.descendant(
        of: inlineAvatar,
        matching: find.byKey(const ValueKey<String>('create-upload-surface')),
      ),
    );
    expect(
      (emptyAvatarSurface.decoration as BoxDecoration).color,
      originWorldDetailSheetBackgroundColor,
    );
    expect(
      tester
          .widget<CustomPaint>(
            find.descendant(
              of: inlineAvatar,
              matching: find.byKey(
                const ValueKey<String>('create-upload-border'),
              ),
            ),
          )
          .painter,
      isA<CreateDashedRRectPainter>().having(
    ...
    final inlineFields = find.descendant(
      of: inlineEditor,
      matching: find.byType(TextField),
    );
    expect(inlineFields, findsNWidgets(3));
    final mountedInlineFields = find.descendant(
      of: find.byKey(
        const ValueKey<String>('origin-setup-role-inline-editor-current-user'),
        skipOffstage: false,
      ),
    ...
    expect(mountedInlineFields, findsNWidgets(3));
    expect(
      tester.getRect(inlineFields.last).bottom,
      lessThanOrEqualTo(tester.getRect(sheetSurface).bottom + 1),
    );
    expect(
      tester.widget<TextField>(inlineFields.at(0)).controller?.text,
      'Profile Hero',
    );
    expect(
      tester.widget<TextField>(inlineFields.at(1)).controller?.text,
      isEmpty,
    );
    expect(
      tester.widget<TextField>(inlineFields.first).focusNode?.hasFocus,
      isTrue,
    );
    final nameTapTarget = find.byKey(
      const ValueKey<String>('origin-setup-role-inline-name-tap-current-user'),
    );
    expect(tester.getSize(nameTapTarget).width, 212);
    expect(tester.getSize(nameTapTarget).height, greaterThanOrEqualTo(33));
    for (final field in tester.widgetList<TextField>(inlineFields)) {
      expect(field.style?.color, originWorldDetailSheetPrimaryTextColor);
      expect(field.decoration?.border, InputBorder.none);
      expect(field.decoration?.enabledBorder, InputBorder.none);
      expect(field.decoration?.focusedBorder, InputBorder.none);
    }
    expect(
      find.descendant(of: inlineEditor, matching: find.text('Name')),
      findsNothing,
    );
    expect(
      find.descendant(of: inlineEditor, matching: find.text('Identity *')),
      findsOneWidget,
    );
    expect(
      find.descendant(
        of: inlineEditor,
        matching: find.text('Personality (Optional)'),
      ),
      findsOneWidget,
    );
    final editingProfileFrame = tester.widget<Container>(
      find.byKey(
        const ValueKey<String>('origin-setup-role-card-frame-current-user'),
      ),
    );
    expect(
      ((editingProfileFrame.foregroundDecoration! as BoxDecoration).border!
              as Border)
          .top
          .color,
      GenesisColors.brand,
    ...
    await tester.pump();
    expect(
      tester.widget<TextField>(mountedInlineFields.at(1)).focusNode?.hasFocus,
      isTrue,
    );
    tester
        .widget<GestureDetector>(
          find.byKey(
            const ValueKey<String>(
              'origin-setup-role-inline-brief-tap-current-user',
            ),
          ),
        )
        .onTap!();
    await tester.pump();
    expect(
      tester.widget<TextField>(mountedInlineFields.at(2)).focusNode?.hasFocus,
      isTrue,
    );
    tester.widget<GestureDetector>(nameTapTarget).onTap!();
    await tester.pump();
    expect(
      tester.widget<TextField>(mountedInlineFields.at(0)).focusNode?.hasFocus,
      isTrue,
    );
    expect(transport.requestsFor('/api/v1/user/info'), isEmpty);
    expect(
    ...
    expect(
      tester
          .getSize(
            find.byKey(
              const ValueKey<String>(
                'origin-setup-role-action-bar-current-user',
              ),
            ),
          )
          .height,
    ...
    await tester.enterText(mountedInlineFields.at(0), 'Temporary Name');
    await tester.enterText(mountedInlineFields.at(1), 'Temporary Identity');
    expect(
    ...
    expect(
      tester.widget<TextField>(reopenedFields.at(0)).controller?.text,
      'Profile Hero',
    );
    expect(
      tester.widget<TextField>(reopenedFields.at(1)).controller?.text,
      isEmpty,
    );
    await tester.enterText(reopenedFields.at(0), 'Outside Tap Draft');
    await tester.tapAt(
      Offset(
        tester.getCenter(sheetSurface).dx,
        tester.getTopLeft(sheetSurface).dy + 24,
      ),
    );
    await tester.pumpAndSettle();
    expect(inlineEditor, findsNothing);
    ...
    tester
        ...
        .onTap!();
    await tester.pumpAndSettle();
    final fieldsAfterOutsideClose = find.descendant(
      of: inlineEditor,
      matching: find.byType(TextField),
    );
    expect(
      tester.widget<TextField>(fieldsAfterOutsideClose.at(0)).controller?.text,
      'Profile Hero',
    );
    tester
        .widget<InkWell>(
  ...
  testWidgets('Origin profile editor launches its custom role inline', (
    ...
    final premountedInlineEditor = find.byKey(
      const ValueKey<String>('origin-setup-role-inline-editor-current-user'),
      skipOffstage: false,
    );
    final premountedFields = find.descendant(
      of: premountedInlineEditor,
      matching: find.byType(TextField, skipOffstage: false),
      skipOffstage: false,
    );
    expect(premountedFields, findsNWidgets(3));
    expect(
      find.byKey(
        const ValueKey<String>('origin-setup-role-inline-editor-current-user'),
      ),
    ...
    final editableTextElementsBeforeEdit = find
        .descendant(
          of: premountedInlineEditor,
        ...
        .evaluate()
        .toList(growable: false);
    expect(editableTextElementsBeforeEdit, hasLength(3));
    ...
    final editableTextElementsAfterEdit = find
        .descendant(of: inlineEditor, matching: find.byType(EditableText))
        .evaluate()
        .toList(growable: false);
    expect(editableTextElementsAfterEdit, hasLength(3));
    for (var index = 0; index < 3; index += 1) {
      expect(
        editableTextElementsAfterEdit[index],
        same(editableTextElementsBeforeEdit[index]),
        reason: 'Edit reveals retained fields instead of mounting new ones.',
      );
    }
    expect(find.text('Identity *'), findsOneWidget);
    expect(find.text('Personality (Optional)'), findsOneWidget);
    ...
    expect(
      ...
      'Initial profile personality',
    );
    await tester.pump();
    final mountedFields = find.descendant(
      of: find.byKey(
        const ValueKey<String>('origin-setup-role-inline-editor-current-user'),
        skipOffstage: false,
      ),
    ...
    expect(mountedFields, findsNWidgets(3));
    expect(nameController.selection, const TextSelection.collapsed(offset: 12));
    expect(
      tester.widget<TextField>(mountedFields.at(1)).controller?.selection,
      const TextSelection.collapsed(offset: 0),
    );
    expect(
      tester.widget<TextField>(mountedFields.at(2)).controller?.selection,
      const TextSelection.collapsed(offset: 0),
    );
    final nameField = tester.widget<TextField>(mountedFields.first);
    expect(nameField.focusNode?.hasFocus, isTrue);
    expect(nameField.style?.fontSize, 14);
    ...
    for (final keyboardInset in <double>[80, 160, 260]) {
      ...
      await tester.pump();
      expect(tester.getTopLeft(sheetSurface).dy, sheetTopBeforeKeyboard);
      expect(nameField.focusNode?.hasFocus, isTrue);
    }
    await tester.pumpAndSettle();
    final keyboardTop = tester.view.physicalSize.height - 260;
    expect(nameField.focusNode?.hasFocus, isTrue);
    expect(
    ...
    await tester.pump();
    expect(
      tester.widget<TextField>(mountedFields.at(1)).focusNode?.hasFocus,
      isTrue,
    );
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final openingComposer = find.descendant(
        ...
        matching: find.byType(LocationChatComposerInput, skipOffstage: false),
        skipOffstage: false,
      );
      expect(openingComposer, findsOneWidget);
      final openingComposerWidget = tester.widget<LocationChatComposerInput>(
        openingComposer,
      );
      expect(openingComposerWidget.sending, isFalse);
      expect(openingComposerWidget.inputEnabled, isFalse);
      expect(
        find.descendant(of: directLaunch, matching: find.text('Launching...')),
        findsOneWidget,
      );
      ...
      final launchBody = transport.decodedBody(
        transport.requestsFor('/api/v1/origin/launch').single,
      );
  ...
  testWidgets('Origin launch enters world without async confirmation polling', (
    ...
    final presetRole = find.byKey(
      const ValueKey<String>('origin-setup-role-c_o_test_1'),
    );
    await _dragOriginPanelUntilVisible(tester, presetRole);
    tester.widget<InkWell>(presetRole).onTap!();
    await tester.pumpAndSettle();
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final visibility = find.byKey(
        const ValueKey('origin-expanded-opening-composer-visibility'),
      );
      expect(visibility, findsOneWidget);
      expect(tester.widget<IgnorePointer>(visibility).ignoring, isFalse);
      final sheetComposer = find.descendant(
        of: find.byKey(const ValueKey('origin-expanded-opening-composer')),
        matching: find.byType(ChatComposer),
      );
      expect(sheetComposer, findsOneWidget);
      final sheetComposerWidget = tester.widget<ChatComposer>(sheetComposer);
      ...
      expect(
        tester
            .widget<Transform>(dockRoleTranslation)
            .transform
            .getTranslation()
            .y,
        -2,
      ...
      final sharedComposerWidget = tester.widget<LocationChatComposerInput>(
        sharedComposer,
      );
      final sharedComposerInput = find.descendant(
        of: sharedComposer,
      ...
      final selectRoleTitleRect = tester.getRect(selectRoleTitle);
      final sheetSurfaceRect = tester.getRect(sheetSurface);
      expect(
        selectRoleTitleRect.top,
        greaterThanOrEqualTo(
          sheetSurfaceRect.top + originDetailSheetHeaderHeightForTesting,
        ),
      );
      expect(selectRoleTitleRect.bottom, lessThan(sheetSurfaceRect.bottom));
      ...
      expect(
        tester.widget<PageView>(rolePages).physics,
        isA<PageScrollPhysics>(),
      );
      ...
      final selectedPresetFrame = find.byKey(
        const ValueKey<String>('origin-setup-role-card-frame-c_o_test_1'),
      );
      final selectedPresetDecoration =
          tester.widget<Container>(selectedPresetFrame).foregroundDecoration!
              as BoxDecoration;
      expect(
      ...
      expect(
        launchedWorldPage.initialMentionCatalog?.entryForId('c_o_test_1'),
        isNotNull,
      );
      ...
      chatroom.session.emit(
        ChatroomUserMessage(
          ...
          ts: null,
          messageId: 43,
          locationMessageId: 43,
          conversationRoundId: 'round-sheet',
          roundOrder: 0,
          senderType: 'user',
          senderId: 'u_mock',
          senderName: 'Me',
  ...
  testWidgets('world Tilemap ignores backend last chat location for focus', (
    WidgetTester tester,
  ) async {
    final transport = _RecordingV1ListTransport(
      worldRelationStatus: 'anonymous',
    ...
    final tilemap = tester.widget<Tilemap>(find.byType(Tilemap));
    expect(tilemap.preferredFocusLocationId, isEmpty);
    expect(
      transport
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final transport = _RecordingV1ListTransport(
        worldRelationStatus: 'anonymous',
        worldDefinitionVersion: 1,
        worldLastChatLocationId: 'loc_l2',
