...
void main() {
  ...
  test(
    'purchase debug details default off and remember an explicit choice',
    () async {
      expect(await purchaseToastDebugSettings.load(), isFalse);
      for (final message in ['Purchase failed.', 'Premium purchase failed.']) {
        expect(
          purchaseToastMessage(message, debugInfo: 'checkout; reason=test'),
          message,
        );
      }
      ...
      for (final message in ['Purchase failed.', 'Premium purchase failed.']) {
        expect(
          purchaseToastMessage(message, debugInfo: 'checkout; reason=test'),
          'debug：checkout; reason=test\n$message',
        );
      }
      purchaseToastDebugSettings.resetForTesting();
      expect(await purchaseToastDebugSettings.load(), isTrue);
      await purchaseToastDebugSettings.setEnabled(false);
      purchaseToastDebugSettings.resetForTesting();
      expect(await purchaseToastDebugSettings.load(), isFalse);
      expect(
        purchaseToastMessage('Purchase failed.', debugInfo: 'checkout'),
        'Purchase failed.',
      );
    },
  );
  ...
  test(
    'API diagnostics include failure metadata without request credentials',
    () async {
      await purchaseToastDebugSettings.setEnabled(true);
      final info = purchaseDebugInfo(
        'vip.report',
