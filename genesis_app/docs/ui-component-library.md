...
所有图形在 20×20 占位中居中：gem、badge 绘制为 20×20；edit、inspiration 按 85% 绘制为 17×17；memory 按 90% 绘制为 18×18；recharge 和未知 key 的兜底升级箭头按占位尺寸绘制。传入其他 size 时保持相同比例。该调整只用于 Subscription 权益列表，不修改 Message 操作栏或共享 SVG。
