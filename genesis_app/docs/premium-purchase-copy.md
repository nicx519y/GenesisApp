...
用户可见的会员称谓统一使用 **Premium**；取消提示统一使用美式拼写 **canceled**。商店错误码、接口字段和诊断标识保留原值。

以下为前端当前使用的英文文案，合并重复项；触发条件仍由原有购买逻辑决定。

## 购买状态

- Premium purchase failed.
- Premium purchase canceled.
- Premium payment is pending.
...
- The store could not verify this Premium purchase.
- The Premium purchase does not match the selected plan.
- This Premium purchase was canceled.
- This Premium purchase was revoked.
- Premium purchase failed.
...
- 宝石购买：Purchase canceled.
- Google 登录：Google sign-in canceled.
- 登录取消异常：Sign-in canceled
- 调试图片生成：Debug image generation was canceled.
