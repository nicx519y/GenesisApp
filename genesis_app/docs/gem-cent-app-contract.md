...
- 保留 500ms 自动保存、对数映射及 1K 步进。标题为 `Max memory token limit`，滑杆下限固定 4000 tokens（4K），上限沿用接口；拖动不预览卡片范围。滑杆显式横向 padding 和气泡共用 24px 轨道留边；气泡主体与尖角始终居中对齐操作柄，包含两端及 RTL；轨道两端到卡片边框各 19px。保存成功后联合获取 World 用量和模型报价，完成后一次更新卡片数据，并检查 World、请求版本与预算一致性。
