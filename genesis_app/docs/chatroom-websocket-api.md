...
本文档按 `/Users/ionix/Downloads/aitown-chat-ws.yaml` 和 `/Users/ionix/Downloads/ws-protocol-changelog.md` 更新，描述 AITown 聊天室 WebSocket `2.0.0` 最新协议，以及 Flutter 侧当前实现需要遵守的字段契约。变更记录中较新的字段调整优先生效。
...
| 版本 | `2.0.0` |
| Dev WS 服务 | `wss://api.worldo.ai/aitown-chat/ws` |
...
```text
GET wss://api.worldo.ai/aitown-chat/ws?world_id={world_id}
```
...
| `sender_id` | `string` | 是 | 当前角色/发送者 ID |
| `sender_name` | `string` | 是 | 当前角色/发送者显示名 |
...
| `user_location_change` | 玩家 join/leave 或收到 `tick_start` | 调用 `/aitown-chat/api/ulocation?world_id=xxx` 拉取角色位置 |
...
- `characters[].char_id`: string，角色 ID
- `characters[].player_uid`: string，真实用户 UID；AI 角色为空字符串
- `characters[].player_username`: string，真实用户名；AI 角色为空字符串
- `characters[].name`: string，角色名称
- `characters[].location_id`: string，当前地点 ID
...
| WebSocket `1xxx` | `1007` | `user_id`、`sender_id`、`sender_name` 必填 |
...
- `join`、`send_message`、`heartbeat`、`leave` 上行消息只使用顶层字段，不发送旧 `payload` 包裹；`join` 必须发送 `user_id`、`sender_id`、`sender_name`。
