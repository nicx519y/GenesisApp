...
| `map_updated` | `payload: {}` | 递增 `WorldChatroomState.mapUpdatedRevision`，并与同批 `world_change`、`tick_done`、`character_updated` 合并刷新一次 world detail；从 `locations` 中筛选新增 ID 且 `is_new=true` 的项后发布顶部 Push 通知 |
...
- `map_updated` 仍发布递增 revision 供 Tilemap 刷新；`map_updated`/`character_updated` 的 detail 结果分别从 `locations`/`characters` 中筛选同时满足“ID 不在刷新前快照中”且 `is_new=true` 的项，生成去重后的顶部 Push 通知。
