...
- 新旧地图的消息气泡统一复用 `WorldMapMessageBubbleSurface`。
- 保留深灰紫半透明底色 `#3A3942`（80% 不透明度）及原浅色文字、字号、圆角和内边距。
- 背景模糊使用 `GenesisBlur.light`（4），仅作用于气泡及尾巴轮廓内。
- 外轮廓为 1px、18% 白色内描边；主体与尾巴使用同一完整轮廓，连接处不绘制横线。
- 指向头像的尾巴宽 12px、高 10px，尖端与头像底部保留 2px 间隙。
- Location name 的底色与样式不受消息气泡调整影响。
