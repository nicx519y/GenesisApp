...
- report 响应必须有合法业务 envelope 和 `data.status`，模型读取 status 及可选的 reason，不读取 report_id、membership_id。允许 completed、accepted、rejected；缺失或未知状态结束本次上报并提示 `Report failed.`，不重试。2026-09-17 按用户要求，rejected 的 reason 用于 Toast：`account_mismatch` 与 Google 身份不一致错误共用 `Account identifiers don't match the previous subscription.`；其他非空 reason 原样显示。缺失、空白或非字符串 reason 使用 `Report failed.`。reason 只用于展示及保留原拒绝结果，不参与 UUID 比较或购买拦截。
...
- Apple 本次 `Product.purchase` 的直接结果通过原生桥接回传本地 `checkoutAttemptId`，只用于对应点击的弹窗收尾，不进入 Apple 购买参数、HTTP 或持久化。系统“已订阅”返回已处理交易时，立即结束本次 loading；原 report 为 rejected/accepted 时保留失败/确认中提示，原 report 技术失败时保留失败提示，原 report completed 时只关闭等待，不重复显示新购买成功，也不重复 report 或迁移订单归属。后台续费、恢复及未完成交易查询不携带该标识，不能关闭正在进行的另一笔购买；迟到的直接结果仍按原点击处理。
