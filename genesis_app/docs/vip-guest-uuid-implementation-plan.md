...
未登录进入主界面时，有已购买缓存就用其中最新一笔的原 UUID 调用 check，不查询商店。没有已购买缓存才只读查询当前 App 的 Google SUBS / Apple 有效订阅，排除待付款及已过期交易，按购买时间选最新一笔，取其原 UUID 调用 check；不遍历所有 UUID。购买时间相同而不能确定最新凭据时，不任意选单。两条路径均以 has_unbound_order 决定是否强制登录；false 是正常结果，不触发重试。check=true 且选中原购买证明时，将原 UUID、商品 ID、token/交易 ID 与稳定 request_id 放入独立游客认领缓存，登录成功后直接进入 claim 重试流程。商店发现不创建恢复订单、不调用 prepare/report，也不要求先打开 VIP 页。游客购买成功后的 OK → 登录顺序保留。绑定完成且 report/平台收尾结束后删除购买凭据、UUID 和归属缓存，停止自动 claim；以后未登录启动从平台重新发现身份再 check。
...
### 2026-09-14 恢复与绑定清理调整

- 调起支付前持久化本次 UUID；仅身份记录不表示付款，不参与 check、登录或 claim。明确取消或拒绝调起后安全删除未成交身份；同 UUID 已有付款凭据、待付款记录或归属信息时保留。超时和未知网络错误保留身份。付款后沿用真实凭据保存和 report，并记录购买时间。
- 商店查询与 check 的异常使用独立超时、最多三次退避重试；首页重入和切前后台不能绕过退避或重置次数。成功结果（包括 false）在 30 秒内复用；切前后台不主动清空结果，新购买及会话变化才使其失效。重复商店回调去重。缺身份订单不能当成无订阅；仅允许按已保存精确交易凭据匹配 UUID。
- claim completed 且 report/平台收尾结束后删除对应 UUID、归属、认领证明及终态 report 残留；旧版已绑定身份缓存同步清理。登录成功本身、check=false 和 claim accepted/rejected 均不代替绑定完成。
- 本地与商店同时缺少原 UUID 时，当前 check 参数不足以恢复，需要后端凭平台交易证明补充身份查询能力。
