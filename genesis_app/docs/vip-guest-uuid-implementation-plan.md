...
- 调起支付前持久化本次 UUID；仅身份记录不表示付款，也不直接触发登录或 claim。付款后沿用真实凭据保存和 report。
- 商店查询与 check 使用独立超时、最多三次退避重试；短期成功结果在回前台、会话和购买状态变化时失效。缺身份订单不能当成无订阅；仅允许按已保存精确交易凭据匹配 UUID。
