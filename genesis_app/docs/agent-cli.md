...
CLI 会连接 App 内部启动的本地 HTTP RPC 服务。这个服务只监听 `127.0.0.1`，默认关闭，只有通过 `./tools/agent_cli/run` 或显式 dart-define 启动时才会开启。
...
`./tools/agent_cli/run` 可以理解为增强版 `flutter run`，它会自动处理：
...
如果需要同时使用 CLI 和 location chat debug dashboard，可以直接运行：
...
- 创建或复用 `.agent-control.env`
- 未指定 `-d` 时，在交互式终端列出可用 Flutter devices 供选择
- 后台启动 dashboard，默认打开 `http://127.0.0.1:17318/`
- 启动 App，并额外开启 `GENESIS_LOCATION_CHAT_DEBUG=true`
- 复用同一个 CLI token，所以另开终端可以继续执行 `./tools/agent_cli/ctl ...`
...
正常情况下不需要手动执行，`./tools/agent_cli/run` 会自动创建。
...
```bash
./tools/agent_cli/ctl app navigate /search
```
...
```bash
./tools/agent_cli/ctl app navigate /search --arg q=alice --replace
```

清空页面栈后打开页面：

```bash
./tools/agent_cli/ctl app navigate /home --clear-stack
```
...
```bash
./tools/agent_cli/ctl world locations --wid world_123
```
...
```bash
./tools/agent_cli/ctl app navigate /location_chat \
  --arg wid=world_123 \
...
```bash
./tools/agent_cli/ctl app navigate /world --arg wid=<WID> --clear-stack
```

示例：

```bash
./tools/agent_cli/ctl app navigate /world --arg wid=world_123 --clear-stack
```
...
- 不知道 `location_id` 时，先执行 `./tools/agent_cli/ctl world locations --wid <WID>`
...
未指定 `--wid` 时，会从当前账号可用的 my worlds 里随机选择一个已 launch/join、可聊天的 world。
未指定 `--location-id` 时，会从该 world 的 leaf locations 里随机选择一个 location。

指定 world：

```bash
./tools/agent_cli/ctl agent world-chat --wid <WID> --count 100
```

指定 world 和 location：

```bash
./tools/agent_cli/ctl agent world-chat \
  --wid <WID> \
...
`./tools/agent_cli/ctl agent world-chat` 默认使用后台任务模式：
...
注意：`Ctrl+C` 停止的是 CLI 进程，不一定能立刻取消 App 内已经启动的后台任务。要显式取消，需要调用内部 RPC `agent.world_chat.cancel` 并传入 `jobId`。目前 `./tools/agent_cli/ctl` 没有暴露单独的取消子命令，后续可以补成：

```bash
./tools/agent_cli/ctl agent world-chat-cancel --job-id <JOB_ID>
```
...
```bash
./tools/agent_cli/ctl config endpoint set \
  --api dev.hushie.ai \
...
```bash
./tools/agent_cli/ctl cache clear --target image
```

清私信缓存：

```bash
./tools/agent_cli/ctl cache clear --target directMessage
```
...
```bash
./tools/agent_cli/agent-run
./tools/agent_cli/genesisctl app ping
```
...
```bash
dart tools/agent_cli/genesisctl.dart app ping
```
